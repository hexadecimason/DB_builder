...
bad_api_df.replace({'api entry':'.hanged.*'}, value = '## Reassigned', regex = True, inplace = True)
bad_api_df.replace({'api entry':'.*(Combined|.dded).*'}, value = '## Combined into other file', regex = True, inplace =True)
bad_api_df.replace({'api entry': '.*isposal.*'}, value = '## Disposal well: no API', regex = True, inplace = True)
bad_api_df.fillna({'api entry':'## empty field/NaN'}, inplace = True)
...
histogram(null_df['% null'], minimum=0, maximum=100, custbuckets="0,25,50,75,99.9,100")


print("\n\t\thistogram of total number null boxes per file:\n")
histogram(null_df['null entries'], minimum=0, maximum=20, custbuckets="1,2,3,4,5,10,20")
